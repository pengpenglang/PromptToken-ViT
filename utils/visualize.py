...
def cls_padding(image, mask, cls_weight, grid_size):
    ...
    draw.rectangle((0, 0, delta_W, delta_H), fill=(189, 195, 199))
    # draw.text((int(delta_W/4),int(delta_H/4)),'token', fill=(0,0,0)) # PIL.Image.size = (W,H) not (H,W)
...
def highlight_grid(image, grid_indexes, grid_size=14):
    ...
    for grid_index in grid_indexes:
        ...
        a.text((y * w + 6, x * h + 6), str(grid_index), fill=(0, 0, 0))
    return image